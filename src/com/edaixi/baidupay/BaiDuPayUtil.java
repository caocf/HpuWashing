...
public class BaiDuPayUtil {
	...
	private void handlepayResult(int stateCode, String payDesc) {
		...
		case PayStateModle.PAY_STATUS_SUCCESS:
			// 需要到服务端验证支付结果
			EventBus.getDefault()
					.post(new OrderListAdapterEvent("WXPaySucess"));
			break;
	...
	private String createOrderInfo(String name, String price, String num) {
		...
		StringBuffer orderInfo = new StringBuffer("currency=1");
		...
		try {
			orderInfo
					...
					.append("&service_code=1&sign_method=1&sp_no="
							+ PartnerConfig.PARTNER_ID + "&sp_request_type=2"
							+ spUnoParam + "&total_amount=" + bigInterest
							+ "&version=2");
		} catch (Exception e) {
			e.printStackTrace();
		}
		StringBuffer orderInfo1 = new StringBuffer("currency=1");
		try {
			orderInfo1
					.append("&goods_desc=")
