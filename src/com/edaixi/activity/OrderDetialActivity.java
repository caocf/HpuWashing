...
public class OrderDetialActivity extends BaseActivity implements
	...
	@ViewInject(R.id.ll_order_detail_clothing_info)
	private LinearLayout ll_order_detail_clothing_info;
			...
			case 8:
				...
				if (fromJson.isRet()) {
					...
					if (fromJsonOrder != null) {
						if (fromJsonOrder.getAmount_text().length() > 1) {
						...
						if (fromJsonOrder.getPay_status() != null
								&& fromJsonOrder.getPay_status().equals("1")) {
							rl_order_pay_info.setVisibility(View.VISIBLE);
							order_detail_pay_type.setText(fromJsonOrder
									.getPay_type());
							tv_order_detail_fee.setText("订单￥"
									+ (Double.parseDouble(fromJsonOrder
											.getOrder_price()) - Double
											.parseDouble(fromJsonOrder
													.getDelivery_fee()))
									+ "+运费￥" + fromJsonOrder.getDelivery_fee()
									+ "-优惠券￥" + fromJsonOrder.getCoupon_paid());
			...
			case TUREMESSAGECLOTHING:
				...
				if (parseClothingDetail != null
					...
					if (parseClothingDetail.size() > 4) {
						...
						tv_show_more_clothing.setClickable(true);
						ll_show_more_clothing.setClickable(true);
						iv_show_more_clothing.setClickable(true);
					} else {
					...
					lv_order_detail_show_clothing.setVisibility(View.GONE);
					tv_show_more_clothing.setVisibility(View.VISIBLE);
					tv_show_more_clothing.setText("暂无衣物详情，待分检后查看");
					tv_show_more_clothing.setClickable(false);
					ll_show_more_clothing.setClickable(false);
					iv_show_more_clothing.setClickable(false);
					lv_order_detail_show_clothing.setVisibility(View.GONE);
					iv_show_more_clothing.setVisibility(View.GONE);
				}
				break;
